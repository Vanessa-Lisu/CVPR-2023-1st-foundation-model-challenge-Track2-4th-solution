...
class XCenterResizeCropAndInterpolation:
    ...
    def get_params(img, scale, ratio):
        ...
        for attempt in range(10):
            ...
            if w <= img.size[0] and h <= img.size[1]:
                ...
                j = np.random.binomial(img.size[0]-w, 0.5)
                return i, j, h, w
...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        tokenizer, num_max_bpe_tokens, task=None,category = None
    ):
        index_files = self.get_index_files(split, task=task, category = category)
...
class RetrievalDataset(BaseDataset):
    @staticmethod
    def get_index_files(split, task=None, category = None):
        if split == "train":
            if category == 'people':
                return (f"{task}_people.train.jsonl",)
            if category == 'car':
                return (f"{task}_car.train.jsonl",)
            else:
            ...
            if category == 'people':
                return (f"{task}_people.val.jsonl",)
            if category == 'car':
                return (f"{task}_car.val.jsonl",)
    ...
    def __getitem__(self, index: int):
        ...
        # print('data_item', data.keys())#['image', 'language_tokens', 'padding_mask', 'image_id']
        return data

    @staticmethod
    def make_flickr30k_dataset_index(data_path, tokenizer, json_path , category):
        ...
        for each_item in captions:
            # image_path = os.path.join("test_images", each_item["filename"])
            split = each_item["split"]
            path = split+"_images"
            image_path = os.path.join(path, each_item["filename"])
        ...
        for split in split2items:
            if category is not None:
                ...
                _write_data_into_jsonl(split2items[split], os.path.join(data_path, "flickr30k_{}.{}.jsonl".format(category, split)))
            else:
                print("%d images and %d image-text pairs!" % (len(split2images[split]), len(split2items[split])))
                _write_data_into_jsonl(split2items[split], os.path.join(data_path, "flickr30k.{}.jsonl".format(split)))
...
def build_transform(is_train, args):
    ...
    if is_train:
        if args.category == 'people':
            t = [
                transforms.Pad((70, 0, 70, 0), fill=0, padding_mode='constant'),
                XCenterResizeCropAndInterpolation(args.input_size, scale=(0.5, 1.0),interpolation=args.train_interpolation),
                transforms.RandomHorizontalFlip(),
        ]
        if args.category == 'car':
            t = [
                RandomResizedCropAndInterpolation(args.input_size, scale=(0.2, 1.0),
