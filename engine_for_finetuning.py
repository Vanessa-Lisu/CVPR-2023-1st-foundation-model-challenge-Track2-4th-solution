...
class RetrievalHandler(TaskHandler):
    ...
    def after_eval(self, **kwargs):
        image_feats = {}
        print("image_ids shape: ", type(self.image_ids))
        ...
        tiids = torch.cat(self.image_ids, dim=0) #拉成一列的image_id
        iids = []
        sorted_tensors = []
        ...
        scores = text_cls_feats @ image_cls_feats.t() #infer
        return scores
...
def train_one_epoch(
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        for tensor_key in data.keys():
            ...
            # print("datainput %s shape %s type %s" % (tensor_key, data[tensor_key].size(), type(data[tensor_key])))
